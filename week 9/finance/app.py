...
def index():
    ...
    # Iterate over stocks and add proce and total value
    for stock in stocks:
    ...
    # Format the values as US dollars and up to 2 decimal places
    cash = "${:,.2f}".format(cash)
    total_value = "${:,.2f}".format(total_value)
    grand_total = "${:,.2f}".format(grand_total)
    for stock in stocks:
        stock["price"] = "${:,.2f}".format(stock["price"])
        stock["value"] = "${:,.2f}".format(stock["value"])
...
def register():
    ...
    # User reached route via POST ie by subbmitting a form via POST
    if request.method == "POST":
        ...
        return redirect("/")
    # User reached route via GET ie by clicking a link or via redirect
    else:
